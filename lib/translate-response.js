...
function translateResponse(responseObj){
 ...
 if (responseLen === 1){
   if(isNaN(Object.keys(responseObj)[0])){
    ...
    return responseObj[name];
      ...
      for(k in responseObj){ // for job.list
         if(responseObj[k] instanceof Buffer){
          responseObj[k]  = responseObj[k].toString('utf8');
         } else { // for session.list 
            for(k2 in responseObj[k]){
              if(responseObj[k][k2] instanceof Buffer){
                responseObj[k][k2] = responseObj[k][k2].toString('utf8')
              }
            }
         }
      }
 ...
 // else module.info, and module.options 
    ...
    // module.options
      ...
      for(k in responseObj[key]){
        ...
        } else if (Array.isArray(responseObj[key][k])) {
          for(var i=0;i<responseObj[key][k].length;i++){
            responseObj[key][k][i] = responseObj[key][k][i].toString('utf8');
          }
        }
      }
    }
