...
const errString = chalk.red('ERROR:')
...
class MsfRpcClient {
  constructor(options) {
    ...
    try
    {
      ...
      {
          throw `${errString} Must have a token or a password`;
      }
      else if (!options.token)
      {
        if (!options.password)
        {
          throw `${errString} If not authenticating with a token, must initialize with username and password`;
        } 
        else
        {
  ...
  _rcpApiCall(cmd) {        
    ...
    // new promise to represent http request
		return new Promise(
			...
			try 
			{
				try{var data = encode(cmdArray);} catch(err){throw `${errString} msgpack encoding error\nDid you enter a password?`}
          ...
          encoding: null
	          ...
	          if (err)
	          {
	            reject(`${errString} request - ${err.code}`);
	          } 
	          else 
	          {
	            ...
	            {
	              reject(`${errString} RPC API: ${translatedStrings.error_message}`);
	            } 
